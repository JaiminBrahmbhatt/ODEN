...
if __name__ == "__main__":
	...
	try:
		...
		print('Experiment1.py -a <value_a> -b <value_b> -p <points> -e <epochs> -l <no.of_layers> -n <no.of_neurons>')
		sys.exit(2)
	for opt, arg in opts:
		if opt == '-h':
	 		print('Experiment1.py -a <value_a> -b <value_b> -p <points> -e <epochs> -l <no.of_layers> -n <no.of_neurons>')
	...
	new_y_predict = y_predict.flatten()
	x_exact = np.linspace(0, 1, points)
	y_exact = exact_sol(x_exact, a,b)
	

	plt.figure(1)
	plt.title(f'a = {a} points = {points} architecture = {dimens} \n Difference = {max(y_exact - new_y_predict)}')
	...
	with open(f'OutputValues/a_{a}.txt', 'a') as f:
		line = f'a = {a}, b = {b}, points = {points}, architecture = {dimens}, Difference = {max(y_exact - new_y_predict)} \n'
		f.write(line)
